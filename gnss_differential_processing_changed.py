...
def file_check(rinex_type, new_type, files):
    ...
    file_type = [
        ...
        if os.path.splitext(fname)[1].lower().endswith(rinex_type)
        and "pppstatic" not in fname
        and "nomark" not in fname
    ]
...
def check_precise_files(obs_date, product_modes=None, output_dir="."):
    """
    Checks if the uncompressed precise orbit and clock files 
    are already present in the target directory, across any of the
    given product tiers (defaults to checking ULT, RAP, and FIN).
    """
    if product_modes is None:
        product_modes = ['ULT', 'RAP', 'FIN']
...
def precise_products_download(obs_date, root, product_types=None):
    """
    ...
    if product_types is None:
        days_old = (datetime.date.today() - obs_date).days
        if days_old >= 18:
            product_types = ['FIN']
        elif days_old >= 2:
            product_types = ['RAP', 'FIN']
        else:
            product_types = ['ULT', 'RAP']
    ...
    for product_type in product_types:
        ...
        for filename in download_files:
            ...
            try:
                ...
                if "CLK" in filename and "05M" in filename:
                    ...
                    fallback_path = os.path.join(root, fallback_filename)
                    try:
        ...
        if success and len(pp_files) == 2:
            return pp_files

        print(f"{product_type} products unavailable for {yyyy_ddd}, trying next tier...")

    print(f"No precise products available for {yyyy_ddd} in any tier ({product_types}).")
    return []
...
def process_base_station(base_obs_path, base_nav_path, out_pos_file, conf_file, extra_files, rxn_path):
    ...
    result = subprocess.run(static_ppp_command, capture_output=True, text=True)

    if result.returncode != 0 or not os.path.exists(out_pos_file) or os.path.getsize(out_pos_file) == 0:
        print(f"RTKLIB command: {' '.join(static_ppp_command)}")
        print(f"RTKLIB stdout:\n{result.stdout}")
        print(f"RTKLIB stderr:\n{result.stderr}")
        out_frame.after(0, printOut(f"RTKLIB Processing failed (return code {result.returncode})."))
        main.update_idletasks()
        return False
...
def base_locate(rxn_path, base_obs_path, base_nav_path, conf_file, root):
    ...
    precise_files = check_precise_files(obs_date, output_dir=root)
    ...
    if precise_files:
        ...
        # (tier auto-selected based on data age: ULT/RAP/FIN)
        ppz_files = precise_products_download(obs_date, root)
    ...
    out_frame.after(0, printOut(f"Failed to determine base station location — check precise product downloads and RTKLIB output above."))
    main.update_idletasks()
    return None
...
def pos_process(pos_path, mark_df):
    """
    Pulls data from the files produced in the Rover Directories. 
    """
    # FIX 1: Look for any .pos files if "events" filtering is failing
    pos = [x for x in os.listdir(pos_path) if x.endswith(".pos")]
    ...
    for e_file in pos:
        ...
        if pos_arr and header:
            posdf = pd.DataFrame(pos_arr, columns=header)
            # Use errors='ignore' in case these columns don't exist in a standard .pos
            posdf.drop(columns=["sdne(m)", "sdeu(m)", "sdun(m)", "age(s)", "ratio"], inplace=True, errors='ignore')

    # Quick safety check: If no data was read, return an empty dataframe early
    if posdf.empty:
        out_frame.after(0, printOut("Warning: No valid tracking points found in .pos files."))
        main.update_idletasks()
        posdf["PointID"] = []
        return posdf
    ...
    for (root,dirs,files) in os.walk(main_path,topdown=True):
        ...
        if len(files) > 0:
            ...
            if ("Base" in root) and (len(files) > 1):
                ...
                if base_inc == 0:
                    ...
                    if base_info is None:
                        out_frame.after(0, printOut(f"Skipping rover processing in {root} — base station location could not be determined."))
                        main.update_idletasks()
                        continue
            ...
            if ("Rover" in root) and (len(files) > 1):
                ...
                nav_list = sorted([
                    f for f in rov_files
                    if os.path.splitext(f)[1].lower().endswith("p") or ".nav" in f.lower()
                ])

                rov_nav_list = [root + "\\" + f for f in nav_list]

                mark_df = pd.DataFrame({"time": [], "name": [], "pos": [], "ant_del": []})

                for rov_obs in obs_list:
                    base_name = os.path.splitext(rov_obs)[0]
                    match = [n for n in nav_list if os.path.splitext(n)[0] == base_name]

                    if not match:
                        out_frame.after(0, printOut(f"No matching nav file for {rov_obs}, skipping."))
                        main.update_idletasks()
                        continue
